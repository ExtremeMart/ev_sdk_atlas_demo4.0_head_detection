...
STATUS SampleAlgorithm::Process(const JiImageInfo &frame, const char* dynamicArgs, JiEvent &event)
{      
    //由于roi配置是归一化的坐标,所以输出图片的大小改变时,需要更新ROI的配置  
    if (frame.nWidth != mConfig.currentInFrameSize.width || frame.nHeight != mConfig.currentInFrameSize.height)
    {
	    SDKLOG(INFO)<<"Update ROI Info...";
        mConfig.UpdateROIInfo(frame.nWidth, frame.nHeight);
    }

    //如果输入的参数不为空且与上一次的参数不完全一致,需要调用更新配置的接口
    if(dynamicArgs != nullptr && mStrLastArg != dynamicArgs) 
    {	
    	mStrLastArg = dynamicArgs;
        SDKLOG(INFO) << "Update args:" << dynamicArgs;
        mConfig.ParseAndUpdateArgs(dynamicArgs);
    }

    cv::Mat yuv = cv::Mat(frame.nHeightStride * 1.5, frame.nWidthStride, CV_8UC1, frame.pData);    
    cv::cvtColor(yuv, mOutputFrame, cv::COLOR_YUV2BGR_NV12);
    mOutputFrame = mOutputFrame.rowRange(0, frame.nHeight).colRange(0, frame.nWidth).clone();

    // 针对整张图进行推理,获取所有的检测目标,并过滤出在ROI内的目标
    std::vector<BoxInfo> detectedObjects;    
    std::vector<BoxInfo> validTargets;    
    // 算法处理    
    mDetector->ProcessImage(frame, detectedObjects, mConfig.algoConfig.thresh);    
    ...
    for (auto &object : validTargets)
    {
        if (mConfig.drawResult)
        {
            ...
            {
                ss.precision(0);
                ss << std::fixed << (mConfig.targetRectTextMap[mConfig.language].empty() ? "" : ": ") << object.score * 100 << "%";
            }
    ...
    {        
        ...
        jTmpValue["name"] = mConfig.targetRectTextMap[mConfig.language];
